...
- 已发布正式版：**v0.8.7**（javstash 冷加载优化 + 预告窗口/预告沉底 + 返回顶部按距离分派，changelog 小节 `8.39` / 修改 96–98；v0.7.x 系列已归档为 README 合并摘要）

- 下一次发布建议：**v0.8.8**（changelog 小节编号 `8.40`，修改序号从 `修改 99` 起）——以 `汉化及修复.md` 尾部模板区备注为准，发布时同步更新那里的三个值。
...
2. **`汉化及修复.md`** **→ 「八、修改记录（增量日志）」**（第 551 行起）
   ...
   - 发布归档时：把「未分版本的临时记录」内容移入新版本小节，**归档时先把所有同一类型的修改合并到一起、只记录最终实现**（不要逐条照抄临时记录），并更新模板区备注里的「小节编号 / 版本号 / 修改序号」为下一个值。
...
- **单文件只能串行修改**：对同一文件的多处编辑必须逐个调用，每次等待上一次编辑结果返回后再执行下一处；禁止在同一消息中并行发出多个针对同一文件的编辑调用——并行更改会导致前一项修改被回退丢失。
