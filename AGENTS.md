...
- 已发布正式版：**v0.9.3**（changelog `8.45` / 修改 110–112）
- 下一次发布：**v0.9.4**（changelog `8.46`，修改从 `113` 起）——以 `汉化及修复.md` 尾部模板区备注为准，发布时同步更新那里的三个值
...
| `AGENTS.md` | 「当前状态」区 | 已发布 / 下一次发布的版本号 + changelog 小节 + 修改序号，随发布同步推进 |
...
1. 完成代码修改 → 同步第一节 6 处版本号 → 补写第二节版本说明。
