...
## 当前状态
- 已发布正式版：**v0.7.9**
- 下一次发布建议：**v0.7.10**（changelog 小节编号 `8.32`，修改序号从 `修改 74` 起）——以 `汉化及修复.md` 尾部模板区备注为准，发布时同步更新那里的三个值。
...
| 文件 | 位置 | 内容 | 说明 |
|---|---|---|---|
| `package.json` | 第 4 行 | `"version": "0.7.9"` | 源码版本号（改动源头） |
| `package-lock.json` | 第 3 行、第 9 行 | `"version": "0.7.9"` | 随 package.json 同步；改完运行 `npm install --package-lock-only` 自动更新，或手动同步两处 |
| `binge.yml` | 第 3 行 | `version: 0.7.9` | **Stash 插件清单版本号，Stash 插件列表展示的就是它，用户最直接可见，务必与 package.json 一致** |
| `README.md` | 第 3 行 | `当前版本 **v0.7.9**` | 仓库首页版本声明 |
| `汉化及修复.md` | 第 3 行 | `> **插件版本**: v0.7.9（…）` | 文档头部版本声明 |

## 二、版本说明：每次发布必须补写的位置

1. **`README.md` → 「汉化版变更说明」**（约第 21 行起）
   - 在当前版本系列小节（现为 `#### v0.7.x`，第 43 行）内追加本次版本的功能/修复 bullet；
   - 若跨大版本（如进入 v0.8.x），新建 `#### v0.8.x` 小节，并把旧的 `v0.7.x` 小节内容归档为合并摘要。

2. **`汉化及修复.md` → 「八、修改记录（增量日志）」**（第 551 行起）
   - 正式改动：新建小节 `### 8.32 v0.7.10 <标题>（修改 74–…）`，**插在文档尾部「⬇️ 最新增量区（更新模板）」（第 4320 行）之前**、紧贴上一个已归档版本小节之后；格式沿用「修改 N：标题 → **文件** / **问题** / **修复** / **验证**」，文首写明版本号；
...
- tag 名必须是 `v*.*.*` 格式（如 `v0.7.10`），否则 release workflow 不触发。
