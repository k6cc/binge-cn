...
- **图片查看器（Lightbox）重构**：改为原生横向滚动 + scroll-snap 轨道（与图库卡片轮播同机制）——触屏跟手拖动/惯性/吸附、两图物理衔接滑动；桌面鼠标拖拽翻页（拖动 ≥200px 松手翻页，轻拖回弹）；修复 slide 缺 `box-sizing: border-box` 导致的图片偏下、切换黑缝、末张可划出黑边、高索引箭头失灵（snap 点与 scrollTo 目标偏差随索引累积）、计数越界（16/15）
...
- **图库逻辑**：详情页图库计数改图标+数字徽章（与首页同款）；图库卡片交互后（手动滑动/点圆点/点开图片）永久停止自动轮播；计数徽章改真 `<button>` 消除 aria-hidden 可达性警告；首页图库卡片 lightbox 撤销上游分页拉全量（保持前 20 张控制负载，计数显示真实总数），`MAX_GALLERY_IMAGES` 过时注释（Bug 7 修复：提升到 500）修正为实际逻辑
