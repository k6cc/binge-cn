...
- **图片查看器（Lightbox）重构**：改为原生横向滚动 + scroll-snap 轨道（与图库卡片轮播同机制）——触屏跟手拖动/惯性/吸附、两图物理衔接滑动、桌面鼠标拖拽翻页；修复 slide 缺 `box-sizing: border-box` 导致的图片偏下、切换黑缝、末张可划出黑边、高索引箭头失灵（snap 点与 scrollTo 目标偏差随索引累积）、计数越界（16/15）
