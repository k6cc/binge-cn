...
- **发现区秒开 + 联合主演卡修复 + 横向滑块三连修（v0.8.9）**：发现流新增 stale-while-revalidate——昨日热门/联合演员种子先渲染卡片（7 天兜底），后台拉新后原位替换，每日首次访问不再白等 10 秒级拉取；联合主演场景不再整卡消失——头条演员配额满时按优先级链（其他库内联合演员 → 未关注演员）选新头条，而非丢弃整个场景（VRKM-1884 案例）。横向滑块（故事栏/演员发现条/标签行）抽为通用 `useHorizontalScroller`：轻刷新后位置归零（压制 Chrome scroll restoration 写回子容器，输入事件判别用户真实滚动，8 秒兜底）；近期窗口切换后右箭头恢复正确显隐（React 重建容器时监听自动重绑，回调 ref + state 化节点）
