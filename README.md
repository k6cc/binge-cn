...
- **关注中页筛选框**：原生 `<select>` 在 Android Chrome 弹系统级居中白底大选择框（与应用深色风格脱节）——抽通用 `SortMenu` 组件（从详情页场景排序泛化），文字栏 + 玻璃拟态浮层菜单，桌面 + 手机行为一致；文字栏固定宽度 10rem、文字靠左，浮层菜单 border-box 锁定同宽（两缘构造性对齐，不受字体渲染影响）；菜单项与筛选栏字号/字体统一（修复 `<button>` 不继承页面字体的上游遗留 bug——菜单项一直渲染为 UA 默认 Arial，与触发按钮的 Segoe UI 字形明显不同，详情页排序菜单同受益）；菜单无勾选列，选中项紫色 `#c084fc` 文字标识；点击有与搜索框一致的包围圈
- **详情页图库**：图库内"←返回"改醒目按钮（深色底 + 蓝色 #6aa9ff 边框/文字）；封面计数徽章去图标只留数字；封面标题 720px 以下 0.57rem、图库标题 0.72rem

#### v0.6.7

- **视频窗口定位重构**：横版视频上移（"新居中点"）从 JS 测量 + `translateY` transform 改为 CSS `object-position: 50% 30%`——封面与视频同规则渲染（封面错位类问题从根上消失）、场景切换零过渡动画、竖版内容天然豁免、全屏恢复仅需一条 CSS 规则；转码预热期全屏横竖判断加 Stash 元数据回退，转码视频首次点全屏即正确锁横屏
- **图片查看器（Lightbox）重构**：改为原生横向滚动 + scroll-snap 轨道（与图库卡片轮播同机制）——触屏跟手拖动/惯性/吸附、两图物理衔接滑动；桌面鼠标拖拽翻页（拖动 ≥200px 松手翻页，轻拖回弹；逻辑抽为 `useDragPaging` hook 与图库卡片共用，松手平滑滚到目标页、落定后才恢复吸附，无"先回弹再滑走"割裂）；修复 slide 缺 `box-sizing: border-box` 导致的图片偏下、切换黑缝、末张可划出黑边、高索引箭头失灵（snap 点与 scrollTo 目标偏差随索引累积）、计数越界（16/15）
- **首页切页位置漂移修复**：开"热门"动态后切到演员/发现页再切回不停留原位置、上划闪现别的卡片——根因是 display:none 期间 TanStack Virtual 的 ResizeObserver 仍派发 0×0 尺寸回调清掉卡片缓存尺寸；现在隐藏期间测量冻结（返回上次已知尺寸）+ 切走保存/切回恢复 scrollTop
- **静音撤销全局联动 + 播放层栈**：各表面（首页卡片/Stories 弹窗/演员详情/PH 播放器）恢复独立静音开关（回归上游行为），新窗口从持久化偏好初始化；新增视觉层栈——更高层覆盖层打开时下层视频自动暂停并拒绝自动播放，任意时刻只有一个声音
- **图库逻辑**：详情页图库计数改图标+数字徽章（与首页同款）；图库卡片交互后（手动滑动/点圆点/点开图片）永久停止自动轮播；计数徽章改真 `<button>` 消除 aria-hidden 可达性警告；首页图库卡片 lightbox 撤销上游分页拉全量（保持前 20 张控制负载，计数显示真实总数），`MAX_GALLERY_IMAGES` 过时注释（Bug 7 修复：提升到 500）修正为实际逻辑；图库卡片补桌面鼠标拖拽翻页（复用 `useDragPaging`：拖拽即停自动轮播、位移超 6px 才捕获指针不影响点击开灯箱）

#### v0.6.5–v0.6.6

- **转码流播放链路**：断线自动断点重连（1s 退避、连续失败 3 次熔断、假结尾识别），不再卡死黑屏或退全屏后从随机 A 点重播、B 点循环复活；锁屏/切后台（>20s）回前台立即重建转码流预热，数据就绪（音画同步门槛）后续播；手势快进快退改绝对时间基准（转码视频从实际播放时间起算），指示器 `5:44` 分钟制；`.mp4` 容器但 MPEG-4/Xvid/WMV 系编码自动走转码（原来直连只有声音）
- **全屏退出稳定性**：退出全屏不再引发视频源重载（src 签名短路 + `video.src` 绝对 URL 误判修复）；连续十几个视频反复全屏/退出不再停播与内容串位（两阶段高度校正）；导航保留全屏前显隐状态；电脑端零暂停直接续播；上划宽屏影片封面直接渲染在最终位置（无 0.3s 滑动动画）
- **PH 保存进度条 + 超时放宽**：保存超时放宽至 4 分钟对齐 daemon 上限；保存按钮内嵌实时进度条（轮询 `/save/progress`，旧版 daemon 降级为文字），配合新版 binge-server 生效
