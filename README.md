...
> 基于 [ordureconnoisseur/binge](https://github.com/ordureconnoisseur/binge) v0.4.0 的中文汉化 + 功能修复分支。当前版本 **v0.7.1**。
...
- **进度条命中区扩容**（v0.7.1）：操作区扩为「进度条带 + 时间码上半行」（时间码纯占位、无任何交互），鼠标/触屏拖动更易命中；竖滑滚动信息流 / 横滑擦洗手势隔离不变
- **交互渐变提示**（v0.7.1）：悬停/拖动时进度条变粗，粗条下方追加同色向下渐变淡出（不遮挡时间码），暗示整块命中区都是进度条交互区；竖屏/横屏、底部导航显示/隐藏、全屏三种状态几何一致
