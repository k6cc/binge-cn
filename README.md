...
- **预告窗口/预告沉底本地日历日期修复（v0.10.0）**：binge 曾用 `toISOString()` 的 UTC 日期当"今天"，UTC+8 等时区每天 0–8 点把当天发布的卡片误判为未来预告沉到底部；新增 `localDateStr()` 统一取浏览器本地日历日期，"最近 N 天"/预告窗口边界同步修正，各时区用户在各自日历上结果正确
...
- **底部导航胶囊弱化态（v0.10.7）**：演员详情/saved/settings 打开时胶囊降透明 0.35、仍可点——单击胶囊 = 收起详情/返回 home，不切 tab（iOS modal intercept 模式），再点一次目标 tab 才导航；saved/settings 底部原本纯空占位的 81px 空间补渲染弱化胶囊；refract 强调色主题下弱化时 pill 0.18s 褪回中性白、恢复回色（"锁定即去色"），默认主题 pill 本就为白、观感不变
- **HLS 流 seek 修复（v0.10.8）**：Stash 的 HLS manifest 不读 `?start=`，转码类视频在该模式下拖动进度条/随机起点时画面从头播、时间码却假跳；现识别 HLS 后走原生 seek（浏览器自行换算段号），进度条拖动、随机起点、手势快进/快退、B 点循环一并生效，转圈指示与段级错误重载同步适配

#### v0.4.0–v0.9.x（合并摘要）

- **剧照预览窗系列（v0.9.x）**：全屏剧照预览 + 双通道剧照获取（r18.dev 官方端点 + 逐号探测回退）、查看器优化、番号行窄屏自适应、简介展开封面变暗、剧照页透明按钮不可点击修复、健壮性与无障碍修复
- **默认数据源 javstash（v0.9.2）**：首次安装/未设置时发现/热门/关注/AddScene 默认使用 javstash.org；forage 仍仅对 stashdb.org 生效
- **加入库弹窗与播放体验（v0.9.8–v0.9.11）**：加入库弹窗窄屏布局、StashDB 档案别名修复、场景加入库系列优化、复制按钮反馈跟随 refract 强调色、Stash 保存筛选 GraphQL 400 修复、窗口缩放防跳播
