...
> 基于 [ordureconnoisseur/binge](https://github.com/ordureconnoisseur/binge) v0.4.0 的中文汉化 + 功能修复分支。当前版本 **v0.6.4**。
...
- **StoryViewer PH 无预览条目改纯封面展示**：Stories 里多数商业片源无 mediabook 预览（preview 代理 404），原来报 `MediaError 4` 错误卡片；现在静默降级为封面静态展示，15 秒计时照常自动推进，无任何报错（完整影片播放交给弹窗播放器，Stories 不再尝试 stream 降级）
- **演员详情页 PH 瓦片封面不显示修复**：老式 phncdn 缩略图 URL 含 `(m=…)(mh=…)` 字面括号，`encodeURIComponent` 不编码括号导致未加引号的 CSS `url(…)` 被括号截断、整条 background-image 声明失效——只有新签名 URL（无括号）能显示的"只有一部有封面"现象。修复：`url("…")` 加引号，一处改动全部恢复
