...
- **StoryViewer PH 无预览条目改纯封面展示**（v0.6.4）：Stories 里多数商业片源无 mediabook 预览（preview 代理 404），原来报 `MediaError 4` 错误卡片；现在静默降级为封面静态展示，15 秒计时照常自动推进，无任何报错（完整影片播放交给弹窗播放器，Stories 不再尝试 stream 降级）
