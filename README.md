...
> 基于 [ordureconnoisseur/binge](https://github.com/ordureconnoisseur/binge) v0.4.0 的中文汉化 + 功能修复分支。当前版本 **v0.9.11**。
...
- **剧照预览窗健壮性与无障碍（v0.9.7）**：隐私模糊开启时「更多」压暗失效修复（模糊与压暗叠加生效）；详情/剧照探测失败静默降级、不再卡「探测中」；预览窗模态焦点管理（打开聚焦、关闭归还，Tab 不再落入被遮住的背景元素）
- **加入库弹窗窄屏布局 + StashDB 档案别名修复（v0.9.8）**：不在库演员档案「又名」不再显示模板原文（`performer.aka` 补插值参数）；加入库弹窗窄屏两档布局——≤720px 收窄居中、演员图片缩小，≤480px 铺满、hero 纵向图片居中、场景统计/图片 URL 左对齐姓名列
- **场景加入库弹窗系列优化（v0.9.9）**：场景与演员加入库分开布局——≤720px 换行、换行空白修复、剧照翻页（仅预览不入库）、封面框 320×216 剧照四周自适应居中、日期/代码输入框长度均衡；代码输入框一键复制番号按钮，变粉反馈跟随「跟随 refract 强调色」主题色
- **复制按钮反馈跟随 refract 亮色变体（v0.9.10）**：开启「跟随 refract 强调色」且主题色偏暗（如深红）时，复制按钮的变粉反馈不再发暗发脏——图标/边框改用主题色亮色变体，背景改用主题色淡底
- **Stash 保存筛选修复 + 窗口缩放防跳播（v0.9.11）**：播放页应用 Stash 保存的筛选不再报「GraphQL HTTP 400 Bad Request」——按 Stash UI 存储形状逐字段转换（演员/标签/数值/布尔/stash id/folder 等全部判据）；播放页调整窗口大小时保持当前影片不跳播——resize 后把滚动位置钉回 active 卡片
- **剧照预览剧照页透明按钮不可点击修复（v0.9.6）**：翻到剧照页后，透明隐藏的番号复制/「更多」按钮在图片上仍可点击——`visibility: hidden` 整树禁用，子按钮 `pointer-events: auto` 不再绕过
- **剧照预览番号行窄屏自适应（v0.9.5）**：极窄屏时「工作室 · 时长」整体换到第二行、番号保持第一行完整不截断；更极端窄屏（<340px）番号与工作室/时长各自按需换行
- **剧照预览简介展开封面变暗（v0.9.4）**：简介点「更多」展开时封面整体变暗（滤镜，不改布局与点击判定），弥补手机端竖屏渐变覆盖不到封面上部的问题；点「收起」恢复原亮度
- **剧照预览查看器优化（v0.9.3）**：封面复刻播放页"新居中点"偏上定位，宽屏自适应铺满全屏；信息栏动态贴封面内容底部（超高侵入、不越过剧照数行），单一渐变延伸到底无断层；番号一键复制（粉闪反馈无提示）、简介「更多/收起」截断展开、文本禁选中；点击空白关闭（图片内容/按钮除外、黑边可关），移动端左右划动翻页优先

#### v0.4.0–v0.8.x（合并摘要）

- **数据源可配置 + 空壳演员修复（v0.8.x）**：发现/热门/Stories/关注/AddScene 可切换任意 stash-box 实例，缓存按源隔离、UI 文案随源显示；演员档案可从数据源补全空壳演员空缺列并挂接库内场景（只填空白、不覆盖）
- **导航与 UI 重做（v0.8.x，合并上游 0.14.x）**：移动端底部导航玻璃胶囊化，全屏/竖屏 UI 布局与显隐动效
- **发现流加载提速（v0.8.x）**：stale-while-revalidate 秒开、javstash 冷加载大幅提速、缓存共享与请求去重；新增预告窗口设置与预告沉底开关
- **发现页交互修复（v0.8.x）**：底部导航滑块跟随、推荐页 A 点防刷新、联合主演卡与横向滑块修复、收藏夹定位与包内顺序播放
- **合并上游 0.12.x–0.13.x**：设置页搜索 + 分组、StashDB 网络降级优化、安全修复（外部 URL 过滤、redirect error）、Docker 命名卷、安装探针、daemon 信任区修复；本地定制全部保留
- **PH 保存超时**：30s → 245s 对齐 daemon 下载预算
- **播放交互**：进度条拖动擦洗 + 命中区扩容；转码流加载指示 + 居中定位修复
- **页面稳定性**：演员详情 tab 切换不抖动、Stories 弹窗居中收缩、设置页窄屏溢出修复
- **i18n 多语言架构**：react-i18next 中英双语，设置页切换，翻译键同步
- **播放体验**：随机时段播放、自动字幕、全屏增强（自动隐藏/横滑/长按倍速）、转码流断线重连、播放层栈 + 静音全局同步
- **图库与图片**：图片查看器原生滚动 + 桌面拖拽翻页、图库卡片自动轮播
- **Stories 与 X 弹窗**：头像行秒开、X 弹窗就地播放 + 保存到 Stash
- **binge-server 集成**：API Key 认证、一键安装、cookies.txt 导入、保存进度条
- **通用 SortMenu 组件**：深色浮层菜单替换原生 select
