...
- **转码流播放链路修复**：播放/暂停期间连接被断（反代超时、ffmpeg 中断）自动从断点重连续播（1s 退避、连续失败 3 次熔断、稳定 5s 归零），不再卡死黑屏或退全屏后从随机 A 点重播、B 点循环复活；流被截断的"假结尾"识别后同样断点重连。手势快进/快退改用绝对时间基准——转码视频从实际播放时间起算（原来显示 0s 起加），指示器改 `5:44` 分钟制（原 `000:00`）。随机时段在直连流（mp4/h264）上恢复生效（上版回归：重头播放）
