...
> 基于 [ordureconnoisseur/binge](https://github.com/ordureconnoisseur/binge) v0.4.0 的中文汉化 + 功能修复分支。当前版本 **v0.7.3**。
...
- **进度条拖动擦洗**：拖动实时显示时间码气泡（加粗纯白、深色圆角底），松手才跳转——转码流硬 seek 安全模式；电脑鼠标悬停进度条实时预览时间码，不点击不跳转
- **进度条命中区扩容**（v0.7.1–v0.7.2）：操作区上下对称扩为「进度条带 ± 时间码半行高」（时间码纯占位、无任何交互），鼠标/触屏拖动更易命中、移动端擦洗不易滑出；竖滑滚动信息流 / 横滑擦洗手势隔离不变
- **移动端点按体验**（v0.7.2）：全局去除浏览器默认点按半透明高亮（tap highlight），点按反馈由元素自身状态承担（进度条变粗等）；悬浮时间码气泡位置上移微调
- **演员详情页 tab 切换稳定性**（v0.7.3）：场景/图库/X 标签切换不再水平抖动（滚动条空间常驻）、不闪空状态文案（加载态前置）、跨 tab 往返不因内容高度差闪跳（内容区最小高度 70dvh 兜底）；保持原有滚动位置不跳
- **转码流加载指示**：转码冷启动 / 硬 seek / 断线重连期间画面中央显示弧形转圈（跟随视频内容区），不再黑屏无反馈
- **居中元素定位修复**：从演员/合集卡进入的首个视频按钮/转圈错位、无文件场景按钮落在屏幕中心——补全 GraphQL 字段 + poster 尺寸兜底 + 测量就绪门控，所有居中元素直达"新居中位置"且无位移动画
- **横滑 seek 指示器**：跟随视频内容区居中，时间格式与进度条统一（`05:23`）
- **交互优化**：全屏拖动进度条期间 UI 不淡出；标题与进度条间距收紧；时间码底部空距、气泡样式多轮微调
