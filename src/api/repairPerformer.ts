...
import { getStashDBBox } from "./stashdb";
...
/// Fill the blank columns on an existing performer. Returns the labels
/// of what was written, empty when there was nothing to fill.
export async function fillPerformerFromStashDB(args: {
    ...
    fill("birth date", "birthdate", local.birthdate, scraped.birthdate);
    fill("death date", "death_date", local.death_date, scraped.death_date);
    fill("country", "country", local.country, scraped.country);
    fill("ethnicity", "ethnicity", local.ethnicity, scraped.ethnicity);
    fill("hair colour", "hair_color", local.hair_color, scraped.hair_color);
    fill("eye colour", "eye_color", local.eye_color, scraped.eye_color);
    fill(
        "measurements",
        "measurements",
        local.measurements,
    ...
    fill("breast type", "fake_tits", local.fake_tits, scraped.fake_tits);
    fill(
        "career length",
        "career_length",
        local.career_length,
    ...
    fill("bio", "details", local.details, scraped.details);
...
export interface RepairResult {
    ...
    linked: number;
    /// Labels of the columns filled in.
    filled: string[];
    ...
    if (result.linked > 0) {
        parts.push(
            result.linked === 1
                ...
                : `attached ${result.linked} scenes you already had`,
        );
    } else if (result.linkFailed) {
        parts.push("could not attach her scenes");
    }
    if (result.filled.length > 0) {
        parts.push(`filled in ${sentence(result.filled)}`);
    }
    if (parts.length === 0) {
