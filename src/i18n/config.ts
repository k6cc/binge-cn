...
// t() 后把 "StashDB" 替换为活动数据源展示名（默认源为 "JAVStash"，
// 其余实例按品牌名显示，如 "StashDB" / "ThePornDB"）。这样 25+ 处
// 带源名的文案（"在 StashDB 上查看"、"StashDB 档案"…）无需逐个传
// interpolation，新增文案也只要照常写 "StashDB" 即可。
...
// 无需重渲染）。失败静默：显示名保持 "JAVStash"，与默认行为一致。
void getActiveSource()
    .then(() => {
        if (activeSourceDisplayName() !== "JAVStash") {
