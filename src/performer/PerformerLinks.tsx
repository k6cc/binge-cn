...
// Single entry point used by both library + StashDB bios. Callers pass
// a flat string[] of URLs — this component handles dedupe + platform
...
export function PerformerLinks({ urls }: PerformerLinksProps) {
    ...
    const platforms = new Map<Platform, string>();
    const other: string[] = [];
            ...
            {PLATFORM_ORDER.map((p) => {
                ...
                return (
                    <a
                        ...
                        className={`binge-profile-link-chip is-${p}`}
...
// URL → platform mapping. Hostname-based so subdomains
// (mobile.twitter.com, www.instagram.com, etc) all match. Returns
// null for anything not in the known set — those flow into the
// "other links" popup.
...
// Stash sometimes stores socials as a bare handle (e.g. "@alice")
// instead of a full URL. The legacy twitter/instagram fields are
// normalized at the call site; for the `urls` array we assume real
// URLs but tolerate missing schemes.
    ...
    return (
        ...
        >
            ...
            >
                ...
                <ul className="binge-other-links-list">
                    {urls.map((u) => (
                        <li key={u}>
                            <a
                                href={u}
                                target="_blank"
                                rel="noreferrer noopener"
                                className="binge-other-links-item"
                            >
                                {displayUrl(u)}
                            </a>
                        </li>
...
// Brand glyphs — simple monochrome SVGs sized to flow with text.
// `currentColor` so they inherit chip colour (active vs inactive).
...
function TwitterIcon() {
    return (
        <svg
            ...
            aria-hidden="true"
        >
            <path d="M18.244 2H21l-6.52 7.45L22 22h-6.78l-4.78-6.27L4.86 22H2.1l6.99-7.99L2 2h6.91l4.32 5.71L18.244 2zm-1.18 18h1.79L7.04 4H5.18l11.88 16z" />
        </svg>
    );
}
...
function TikTokIcon() {
    return (
        <svg
            ...
            aria-hidden="true"
        >
            <path d="M20 8.3a7.7 7.7 0 0 1-4.5-1.45v8.4a5.65 5.65 0 1 1-5.65-5.65c.32 0 .63.03.93.08v2.85a2.83 2.83 0 1 0 1.97 2.72V2h2.78a4.78 4.78 0 0 0 4.47 4.47V8.3z" />
        </svg>
    );
}
...
function RedditIcon() {
    return (
        <svg
            ...
            aria-hidden="true"
        >
            <path d="M22 12.07a2.18 2.18 0 0 0-3.7-1.55c-1.5-1-3.5-1.65-5.7-1.73l1.1-3.6 3.2.75a1.6 1.6 0 1 0 .15-.95l-3.6-.85a.48.48 0 0 0-.57.32l-1.27 4.13c-2.27.05-4.34.7-5.9 1.73A2.18 2.18 0 1 0 3.3 13.7a4 4 0 0 0-.05.7c0 3.55 4 6.42 8.9 6.42s8.9-2.87 8.9-6.42c0-.24-.02-.47-.05-.7A2.18 2.18 0 0 0 22 12.07zM7.7 13.45a1.4 1.4 0 1 1 1.4 1.4 1.4 1.4 0 0 1-1.4-1.4zm7.6 4.05c-1.05.62-2.4.96-3.95.96s-2.9-.34-3.95-.96a.55.55 0 1 1 .57-.95c.84.5 2 .77 3.38.77s2.54-.27 3.38-.77a.55.55 0 1 1 .57.95zm-.6-2.65a1.4 1.4 0 1 1 1.4-1.4 1.4 1.4 0 0 1-1.4 1.4z" />
        </svg>
    );
}
...
function OnlyFansIcon() {
    return (
        <svg
            ...
            aria-hidden="true"
        >
            <path d="M 24 4.003 h -4.015 c -3.45 0 -5.3 0.197 -6.748 1.957 a 7.996 7.996 0 1 0 2.103 9.211 c 3.182 -0.231 5.39 -2.134 6.085 -5.173 c 0 0 -2.399 0.585 -4.43 0 c 4.018 -0.777 6.333 -3.037 7.005 -5.995 Z M 5.61 11.999 A 2.391 2.391 0 0 1 9.28 9.97 a 2.966 2.966 0 0 1 2.998 -2.528 h 0.008 c -0.92 1.778 -1.407 3.352 -1.998 5.263 A 2.392 2.392 0 0 1 5.61 12 Z m 2.386 -7.996 a 7.996 7.996 0 1 0 7.996 7.996 a 7.996 7.996 0 0 0 -7.996 -7.996 Z m 0 10.394 A 2.399 2.399 0 1 1 10.395 12 a 2.396 2.396 0 0 1 -2.399 2.398 Z" />
        </svg>
    );
}
