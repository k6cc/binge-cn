    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!alive) return;
                if (!performer) {
                    throw new Error("在 StashDB 上未找到该演员。");
                }
                setState({
                    kind: "ready",
                    performer,
...
function BioAttrs({
    ...
    if (performer.eyeColor) {
        attrs.push(`${performer.eyeColor.toLowerCase()}色眼睛`);
    }
